...
def compute_log_posterior_predictive_linear_gaussian(test_observations: np.ndarray,
    ...
    sampled_posterior = inference_alg.sample_params_for_predictive_posterior(
        num_samples=num_samples)
    indicators_probs = sampled_posterior['indicators_probs']  # shape: (num samples, max num features)
    features = sampled_posterior['features']  # shape: (num samples, max num features, obs dim)
